...
<body>
    <div class="container">
        ...
        <h2 style="margin-bottom: 15px; color: #e0e6e3; font-weight: 600;">Последние визиты (до 20)</h2>
        ...
        <table class="history-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="time-col">{{{{ visit.timestamp.strftime('%Y-%m-%d %H:%M:%S') }}}}</td>
                    <td class="ip-badge">{{{{ visit.ip_address }}}}</td>
                    <td class="agent-col">{{{{ visit.user_agent }}}}</td>
                </tr>
                {{% else %}}
                <tr>
                    <td colspan="4" style="text-align: center; color: #8b9b92; padding: 20px;">Записей пока нет</td>
                </tr>
                {{% endfor %}}
            </tbody>
        </table>
...
# 📊 Новый маршрут для просмотра истории визитов
@app.route('/history')
def history():
    # Получаем 20 последних записей из таблицы visit
    recent_visits = Visit.query.order_by(Visit.timestamp.desc()).limit(20).all()
